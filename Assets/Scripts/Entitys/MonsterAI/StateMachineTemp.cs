...
public class MonsterStateMachineTemp
{
    ...
    public MonsterStateMachineTemp(Astar astar, MonsterEntity myEntity)
    {
        pathFindger = astar;
    ...
    public Vector2Int Get_Destination(MapEntity target)
    {
        ...
        return states[currentState].Get_Destination(myPos, targetPos);

    }
    // [핵심 조립 포인트] 플로우차트의 흐름을 그대로 구현한 상태 결정 함수
    public void UpdateMonsterState(MapEntity target)
    {
    ...
    public bool IsHearSound(MapEntity target)
    {
        bool isHear = false;
        if (target is PlayerEntity player)
        {
            float soundRange = player.GetEntityStat(ModifierTriggerType.Passive)[StatType.Sound];
    ...
    public bool IsAttackable(MapEntity target)
    {
    ...
    public bool IsInVision(MapEntity target)
    {
        if (target == null) return false;
